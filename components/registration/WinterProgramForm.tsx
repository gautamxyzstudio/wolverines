...
export default function WinterProgramForm() {
  ...
  // Form Submit Handler matching /api/pending-winter-camp payload
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(API_ENDPOINTS.PENDING_WINTER_CAMP, {
        method: "POST",
      ...
      if (!response.ok) {
        let errorMsg = resData?.message || "Failed to create winter camp registration.";
        if (resData?.errors) {
  ...
  return (
    <div className="w-full bg-white text-neutral-900 pb-20">
      ...
      <BannerSlider
        slides={[
          { src: "/images/Slider_one.webp",         alt: "Wolverines Winter Program - Slide 1" },
          { src: "/images/Slider_two-300x109.webp",  alt: "Wolverines Winter Program - Slide 2" },
        ]}
      />
        ...
        {successInfo ? (
          <div className="max-w-2xl mx-auto p-8 bg-green-50 border border-green-200 rounded-2xl text-center shadow-xs">
            ...
            <p className="text-neutral-600 text-sm sm:text-base leading-relaxed mb-6">
              Thank you for registering for The Wolverines Winter Program! A confirmation has been recorded.
            </p>
